...
export async function generateQuoteHTML(quote: Quote): Promise<string> {
  ...
  return templateType === 'professional' 
    ? generateProfessionalTemplate(quote, approveUrl, qrCodeDataURL, isInvoice)
    : generateSimpleTemplate(quote, approveUrl, qrCodeDataURL, isInvoice)
}

function generateSimpleTemplate(quote: Quote, approveUrl: string, qrCodeDataURL: string, isInvoice: boolean = false): string {
  // קביעת כותרות לפי סוג המסמך
  const docTitle = isInvoice ? 'חשבון עסקה' : 'הצעת מחיר'
  const docNumberLabel = isInvoice ? 'מספר חשבון:' : 'מספר הצעת מחיר:'
  const approvalText = isInvoice ? 'לחץ כאן לתשלום' : 'לחץ כאן לאישור ותשלום מקדמה'
  const approvalTitle = isInvoice ? 'לתשלום החשבון סרקו את הברקוד או לחצו כאן' : 'לאישור ההצעה סרקו את הברקוד או לחצו כאן'
  
  // חישוב יתרה לתשלום עבור חשבון
  const paidAmount = quote.paidAmount || 0
  const remainingAmount = quote.total - paidAmount
  
  // שם הלקוח - מליד או מלקוח
  const customerName = quote.lead?.name || quote.client?.name || ''
  const customerEmail = quote.lead?.email || quote.client?.email || ''
...
<body>
  <div class="header">
    ...
    <div class="subtitle">${docTitle}</div>
  </div>
  
  <div class="content">
    <div class="info-section">
      <div class="info-row">
        <span class="info-label">${docNumberLabel}</span>
    ...
    <div class="approval-section">
      ...
      <div class="approval-content">
        ...
        <div class="approval-text">
          <a href="${approveUrl}" class="approval-link" target="_blank">${approvalText}</a>
        </div>
      </div>
    </div>
    `}
    ...
    <div class="summary" style="margin-top: 20px; background: #fef3c7; border: 1px solid #f59e0b;">
      <div class="summary-row">
        <span class="summary-label">שולם עד כה:</span>
        <span class="summary-value currency" style="color: #059669;">${formatCurrency(paidAmount)}</span>
      </div>
      <div class="summary-row total-row" style="border-top-color: #f59e0b;">
        <span class="total-label" style="color: #d97706;">יתרה לתשלום:</span>
        <span class="total-value currency" style="color: #d97706;">${formatCurrency(remainingAmount)}</span>
      </div>
    </div>
    ` : ''}

    <div class="footer">
      <div>${isInvoice ? 'חשבון זה' : 'הצעת מחיר זו'} הופק/ה על ידי <span class="brand-name">Quick invoice</span></div>
    </div>
  </div>
</body>
  </html>
...
function generateProfessionalTemplate(quote: Quote, approveUrl: string, qrCodeDataURL: string, isInvoice: boolean = false): string {
  // קביעת כותרות לפי סוג המסמך
  const docTitle = isInvoice ? 'חשבון עסקה' : 'הצעת מחיר'
  const docNumberLabel = isInvoice ? 'חשבון מספר' : 'הצעת מחיר מספר'
  const approvalText = isInvoice ? 'לחץ כאן לתשלום' : 'לחץ כאן לאישור ותשלום מקדמה'
  const approvalTitle = isInvoice ? 'לתשלום החשבון סרקו את הברקוד או לחצו כאן' : 'לאישור ההצעה סרקו את הברקוד או לחצו כאן'
...
<body>
  ...
  <div class="content">
    <div class="doc-info">
      ...
      <div class="doc-number">${docNumberLabel} ${quote.quoteNumber}</div>

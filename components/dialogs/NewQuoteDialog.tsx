...
import { Plus, X, Trash2, BookOpen, Mail, UserPlus } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
interface NewQuoteDialogProps {
  ...
  clientId?: string
  projectId?: string
  duplicateFrom?: DuplicateQuote | null
}
...
export function NewQuoteDialog({
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.settings?.quoteDefaults) {
          return data.settings.quoteDefaults
        }
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (open) {
      ...
      // אם יש נתוני שכפול, טען אותם
      if (duplicateFrom) {
        setFormData({
          leadId: duplicateFrom.leadId || "",
          title: duplicateFrom.title,
          description: duplicateFrom.description || "",
          templateType: (duplicateFrom.templateType || "simple") as "simple" | "professional",
          validUntil: "",
          notes: duplicateFrom.notes || "",
          terms: duplicateFrom.terms || "",
          discount: duplicateFrom.discount || 0,
          tax: duplicateFrom.tax || 18,
          depositPercent: (duplicateFrom as any).depositPercent || 40,
        })
        setItems(
          duplicateFrom.items.length > 0
        ...
        // טעינת ברירות מחדל מהגדרות החברה
        fetchCompanyDefaults().then((defaults) => {
          if (defaults) {
            setFormData((prev) => ({
              ...prev,
              leadId: leadId || "",
              tax: defaults.tax ?? 18,
              depositPercent: defaults.depositPercent ?? 40,
            }))
          } else if (leadId) {
  ...
  const handleLoadTemplate = (template: any) => {
    setFormData({
      ...
      tax: template.tax || 18,
      depositPercent: template.depositPercent || 40,
    })
    setItems(
  ...
  const handleCreateLead = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        await fetchLeads()
        // בחירת הליד החדש
        setFormData((prev) => ({ ...prev, leadId: newLead.id }))
        // איפוס הטופס וסגירה
        setNewLeadData({ name: "", email: "", phone: "" })
        setShowNewLeadForm(false)
        toast({
          title: "הצלחה",
          description: `הליד "${newLead.name}" נוצר בהצלחה`,
        })
      } else {
      ...
      toast({
        title: "שגיאה",
        description: "לא הצלחנו ליצור את הליד",
        variant: "destructive",
      })
    } finally {
  ...
  const validateForm = () => {
    ...
    // אם יש projectId או clientId, לא צריך leadId
    if (!formData.leadId && !clientId && !projectId) {
      toast({
        title: "שגיאה",
        ...
        <DialogHeader>
          <div className="flex items-center gap-3">
            <DialogTitle className="text-2xl font-bold">
              {duplicateFrom ? "שכפול הצעת מחיר" : "הצעת מחיר חדשה"}
            </DialogTitle>
            <Button
              type="button"
              variant="outline"
        ...
        <form onSubmit={handleSubmit} className="space-y-6" style={{ wordBreak: 'break-word', overflowWrap: 'break-word' }}>
          ...
          <div className="border-t pt-4">
            <div className="grid grid-cols-3 gap-4 mb-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="depositPercent">מקדמה (%)</Label>
                <Input
                  id="depositPercent"
                  type="number"
                  min="0"
            ...
            <div className="bg-zinc-50 p-4 rounded-lg space-y-2">
              ...
              <div className="border-t pt-2 mt-2">
                ...
                {formData.depositPercent > 0 && (
                  <div className="flex justify-between text-sm mt-2 text-green-600">
                    <span>מקדמה ({formData.depositPercent}%):</span>
                    <span className="font-medium">
                      ₪{(total * formData.depositPercent / 100).toLocaleString("he-IL", { minimumFractionDigits: 2 })}
                    </span>
                  </div>
                )}
              </div>

...
async function handleCallback(request: NextRequest) {
  try {
    ...
    if (isSuccess && wasNewPayment) {
      ...
      // מציאת משתמש מנהל בחברה (אם אין userId ספציפי)
      let automationUserId: string | null = quote.createdBy || null;
      if (!automationUserId) {
      ...
      try {
        if (quote.leadId && quote.lead && !quote.lead.clientId) {

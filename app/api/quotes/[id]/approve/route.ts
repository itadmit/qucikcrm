...
export async function POST(
  ...
  try {
    ...
    // הערה: התראה על אישור הצעה תישלח רק לאחר השלמת התשלום
    // (ב-callback של PayPlus או Invoice4U)
...
export async function GET(
  ...
  try {
    ...
    // בדיקת אינטגרציות תשלום פעילות של החברה
    const integrations = await prisma.integration.findMany({
      where: {
        companyId: quote.company.id,
        type: {
          in: ['PAYPLUS', 'INVOICE4U'],
        },
        isActive: true,
      },
      select: {
    ...
    return NextResponse.json({
      ...
      isInvoice: quote.isInvoice || false,

...
import { notifyPaymentReceived, notifyQuoteApproved } from "@/lib/notification-service"
...
export async function POST(req: NextRequest) {
  try {
      ...
      if (lead) {
        ...
        if (clientId) {
          ...
          if (existingProject) {
            ...
            if (isSuccess) {
              // מציאת userId לאוטומציות
              let automationUserId = quote.createdBy || null;
              if (!automationUserId) {
                ...
                automationUserId = companyAdmin?.id || null;
              }
    ...
    if (payment) {
      ...
      console.log(`Created payment ${payment.id}, linked to project ${projectId} (client: ${payment.clientId})`)
    }
    ...
    if (isSuccess && wasNewPayment) {
      ...
      if (wasNewPayment && isSuccess) {
        try {
          ...
          // מציאת הבעלים של הליד לשליחת התראה
          ...
          // שליחה לבעלים של הליד
          const notificationUserId = leadOwnerId
          
          // שליחת התראה גם ליוצר ההצעה אם שונה מבעלים של הליד
          const quoteCreatorId = quote.createdBy
          const shouldNotifyCreator = quoteCreatorId && quoteCreatorId !== leadOwnerId
          ...
          // שליחת התראה גם ליוצר ההצעה (אם שונה מבעלים הליד)
          if (shouldNotifyCreator) {
            await notifyPaymentReceived({
              userId: quoteCreatorId,
              companyId: quote.companyId,
              paymentId: payment.id,
              amount: payment.amount,
              quoteNumber: quote.quoteNumber,
              clientName: clientName,
              transactionId: payment.transactionId || undefined,
              sendEmailToAllManagers: false, // לא שולח לכל המנהלים כי כבר נשלח
            })
            console.log(`✅ Payment notification also sent to quote creator`)
          }

          // שליחת התראה על אישור הצעה (אחרי התשלום!)
          if (quote.lead) {
            ...
            console.log(`✅ Quote approved notification sent for quote ${quote.quoteNumber}`)
            
            // שליחת התראה על אישור גם ליוצר ההצעה (אם שונה)
            if (shouldNotifyCreator) {
              await notifyQuoteApproved({
                userId: quoteCreatorId,
                companyId: quote.companyId,
              ...
              console.log(`✅ Quote approved notification also sent to quote creator`)
            }
          }
        } catch (error) {

...
import { Plus, Zap, Mail, UserPlus, Calendar, FileText, MoreVertical, Play, Pause, Edit, Trash2, Clock, FolderPlus } from "lucide-react"
...
export default function AutomationsPage() {
  ...
  const getTriggerIcon = (trigger: any) => {
    ...
    if (lowerTrigger.includes('project')) return FolderPlus
  ...
  const getActionLabel = (actions: any) => {
    ...
    const actionStr = typeof action === 'object' ? action.type || 'פעולה' : String(action)
    const actionLabels: Record<string, string> = {
      'send_email': 'שלח אימייל',
      'create_task': 'צור משימה',
      'create_task_kit': 'צור מספר משימות',
      'send_notification': 'שלח התראה',
      'update_field': 'עדכן שדה',
      'update_status': 'עדכן סטטוס',
      'add_tag': 'הוסף תגית',
    }
    let label = actionLabels[actionStr] || actionStr
    if (actionStr === 'create_task' && action?.params?.taskTitle) {
      label += `: ${action.params.taskTitle}`
      if (action.params.checklist?.length) {
        label += ` (${action.params.checklist.length} פריטי צ׳קליסט)`
      }
    }
    return label
  }

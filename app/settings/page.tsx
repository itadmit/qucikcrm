...
export default function SettingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"general" | "quotes" | "communication" | "security" | "advanced">("general")
  ...
  const handleSaveProfile = async () => {
    if (!session?.user?.id || !profileName.trim()) return
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: "הפרופיל עודכן!",
          description: "השינויים נשמרו בהצלחה. יש להתחבר מחדש לראות את השינויים.",
        })
        ...
        toast({
          title: "שגיאה",
          description: error.error || "לא ניתן לעדכן את הפרופיל",
          variant: "destructive",
        })
      }
    } catch (error) {
  ...
  // Quote defaults settings
  const [quoteDefaults, setQuoteDefaults] = useState({
    depositPercent: 40,
    tax: 18,
    validityDays: 30,
  })
  const [loadingQuoteDefaults, setLoadingQuoteDefaults] = useState(false)
  ...
  const sidebarPermissions = [
    ...
    { key: "attendance", label: "שעון נוכחות", required: false },
  ...
  // Load quote defaults when tab is activated
  useEffect(() => {
    if (activeTab === "quotes") {
      fetchQuoteDefaults()
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.settings?.quoteDefaults) {
          setQuoteDefaults({
            depositPercent: data.settings.quoteDefaults.depositPercent ?? 40,
            tax: data.settings.quoteDefaults.tax ?? 18,
            validityDays: data.settings.quoteDefaults.validityDays ?? 30,
          })
        }
      }
    } catch (error) {
      ...
      setLoadingQuoteDefaults(false)
    }
  }

  const handleSaveQuoteDefaults = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: "נשמר בהצלחה",
          description: "הגדרות ברירת המחדל להצעות מחיר עודכנו",
        })
      } else {
        throw new Error('Failed to save')
      }
    } catch (error) {
      console.error('Error saving quote defaults:', error)
      toast({
        title: "שגיאה",
            ...
            <CardContent>
              {isEditingProfile ? (
                <div className="space-y-4">
                  <div>
                    <Label htmlFor="profileName">שם</Label>
                    <Input
                      id="profileName"
                      value={profileName}
              ...
              <CardHeader>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription>הגדרות אלו ישמשו כערכי ברירת מחדל בעת יצירת הצעת מחיר חדשה</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
                {loadingQuoteDefaults ? (
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-3">
                        <Input
                          ...
                          max="100"
                          step="1"
                          value={quoteDefaults.depositPercent}
                          onChange={(e) => setQuoteDefaults({
                            ...quoteDefaults,
              ...
              <CardHeader>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription>איך זה עובד?</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="bg-white p-3 rounded border border-blue-200 mb-3">
                      ...
                      <ul className="text-xs text-gray-600 space-y-1 grid grid-cols-2 gap-x-4">
                        ...
                        <li>• 3 הצעות מחיר עם פריטים</li>
                        <li>• 4 תבניות הצעות מחיר</li>
                        <li>• 6 תשלומים (שולם/ממתין)</li>
                        <li>• 3 תבניות משימות</li>
                        <li>• 5 תקציבים</li>
                        <li>• 5 פגישות וארועים</li>
                        <li>• 3 תבניות אימייל</li>
                        <li>• 3 אוטומציות</li>
                        <li>• 4 התראות</li>
